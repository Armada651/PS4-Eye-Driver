...
#define LIBUSB_ENDPOINT_OUT 0x0
...
#define LIBUSB_RECIPIENT_OTHER      0x03
    ...
    public:
        ...
        static int listDevices(std::vector<PS4EYECam::PS4EYERef>& list, const GUID* device_interface, bool firmware_check);
    ...
    int USBMgr::listDevices(std::vector<PS4EYECam::PS4EYERef>& list, const GUID* device_interface, bool firmware_check)
    {
        SP_DEVINFO_DATA dev;
        ...
        cnt = 0;
        for (int i = 0; SetupDiEnumDeviceInterfaces(devs, NULL, device_interface, i, &intf); i++)
        {
            // get string size
            DWORD size = 0;
            ...
            {
                list.push_back(PS4EYECam::PS4EYERef(new PS4EYECam(handle, firmware_check)));
                cnt++;
            }
        }
    ...
    {
    public:
        URBDesc(WINUSB_INTERFACE_HANDLE handle, uint8_t mode): handle_(handle), num_transfers(0), last_packet_type(DISCARD_PACKET), last_pts(0), last_fid(0), xfr(), xfr_handle()
        {
            is_streaming=false;
        ...
        URBDesc(WINUSB_INTERFACE_HANDLE handle) : handle_(handle), num_transfers(0), last_packet_type(DISCARD_PACKET), last_pts(0), last_fid(0), xfr(), xfr_handle()
        {
        ...
        bool start_transfers(uint32_t curr_frame_size)
        {
			...
			for (int i = 0; i < ISOCH_TRANSFER_COUNT; i++)
				xfr_handle[i].hEvent = CreateEvent(NULL, FALSE, FALSE, NULL);
            ...
            num_transfers = ISOCH_TRANSFER_COUNT;
            frame_complete_ind = 0;
            frame_work_ind = 0;
            last_pts = 0;
            last_fid = 0;
        ...
        {
            WinUsb_AbortPipe(handle_, 0);
            /*while(num_transfers)
            {
                if( !USBMgr::instance()->handle_events() || num_transfers==0 )
                {
                    break;
                }
            }*/
			for (int i = 0; i < ISOCH_TRANSFER_COUNT; i++)
				CloseHandle(xfr_handle[i].hEvent);
        }
        ...
        void handle_events(HANDLE device)
        {
            for (int i = 0; i < ISOCH_TRANSFER_COUNT; i++)
            {
                DWORD actual_length = 0;
                if (GetOverlappedResult(device, &xfr_handle[i], &actual_length, FALSE))
                {
                    if (ff71status<0)
                    {
                        ff71status++;
    ...
    const std::vector<PS4EYECam::PS4EYERef>& PS4EYECam::getDevices( bool forceRefresh )
    {
        ...
        if (USBMgr::instance()->listDevices(devices, &GUID_DEVINTERFACE_PS4EYE, true) > 0)
			std::cout << "Found PlayStation Camera" << std::endl;


		if (USBMgr::instance()->listDevices(devices, &GUID_DEVINTERFACE_USBBOOT, false) > 0)
    ...
    PS4EYECam::PS4EYECam(HANDLE device, bool firmware_check)
		: control_transfer()
    {
        // default controls
    ...
    void PS4EYECam::multia7_register_write(const uint16_t data[][3], uint8_t len)
    {
        ...
        /* int size= ARRAY_SIZE(data);
         debug("size es %d\n",size);*/
        uint8_t *buffer_out=new unsigned char[len*4+8]();
        buffer_out[0]=0x02;
        for(i=0;i<len;i++)
        {
            buffer_out[8+(4*i)+1]=((data[i][0])>>8)&0xff;
            buffer_out[8+(4*i)]=(data[i][0])&0xff;
            buffer_out[8+(4*i)+2]=data[i][1];
            buffer_out[8+(4*i)+3]=data[i][2];
        }
        debug("USB HEADER\n");
        for(i=0;i<8;i++)
        {
    ...
    void PS4EYECam::register_write(uint16_t reg, uint8_t val,uint8_t subaddr)
    {
        ...
        buffer_out[9]=(reg>>8)&0xff;
        buffer_out[8]=reg&0xff;
        buffer_out[10]=val;
        buffer_out[11]=subaddr;
...
*/
    ...
    bool PS4EYECam::init(uint8_t initmode, uint8_t desiredFrameRate)
    {
        ...
        urb = std::shared_ptr<URBDesc>(new URBDesc(handle_, mode));

        frame_stride = frame_width * 2;
		/* FIXME: Why do these sanity checks fail?
        if(!check_sensors())
        {
            debug("Sanity check from sensors failed\n");
            //set_led_off();
    ...
    void PS4EYECam::start()
    {
       ...
       //TODO define different  modes to choose resolution and fps
        ...
        urb->start_transfers(frame_stride*frame_height);
...
*/
    ...
    int PS4EYECam::uvc_get_hue(int16_t *hue, uint8_t req_code){
        ...
        ret = submitAndWait_controlTransfer(REQ_TYPE_GET, req_code,UVC_PU_HUE_CONTROL << 8,1 << 8,sizeof(data),data);
    ...
    int PS4EYECam::uvc_set_hue(int16_t hue){
        ...
        ret = submitAndWait_controlTransfer(
   	...
   	int PS4EYECam::uvc_set_video_mode(uint8_t mode,uint8_t fps)
	{
		...
		err = submitAndWait_controlTransfer(
                                      0x21,
                                      0x01,
                                      (0x01 << 8) ,
    ...
    {
        // open
        BOOL res = WinUsb_Initialize(device_, &handle_);
        if(!res) {
        ...
        //claim interface 1
        base_handle_ = handle_;
        res = WinUsb_GetAssociatedInterface(base_handle_, 0, &handle_);
        if(!res) {
        ...
        //res=WinUsb_SetCurrentAlternateSetting(handle_, 0);
     ...
     */
    uint8_t * PS4EYECam::controlTransferStatus(DWORD x)
    {
        switch (x)
        {
            case NO_ERROR:
     ...
     * been processed.
     */
    int PS4EYECam::submitAndWait_controlTransfer(uint8_t bmRequestType, uint8_t bRequest, uint16_t wValue, uint16_t wIndex, uint16_t wLength, uint8_t *buffer)
    {
    ...
    void PS4EYECam::firmware_upload()
    {
        if(!firmwareisloaded)
        {
            ...
            std::ifstream firmware(firmware_path, std::ios::in|std::ios::binary|std::ios::ate);
            if (firmware.is_open())
            {
